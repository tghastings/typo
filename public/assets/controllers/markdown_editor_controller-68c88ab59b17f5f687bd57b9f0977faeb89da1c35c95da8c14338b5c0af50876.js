...
export default class extends Controller {
  ...
  insertTypoPdf() {
    const src = prompt("Enter PDF filename (from /files/) or full URL:", "")
    if (!src) return

    const title = prompt("Enter presentation title (optional):", "")

    let attrs = `src="${src}"`
    if (title) attrs += ` title="${title}"`

    this.insertText(`<typo:pdf ${attrs}/>`)
  }
  ...
  processTypoMacrosForPreview(html) {
    ...
    html = html.replace(/&lt;typo:lightbox[^\/]*\/&gt;/g,
      '<span style="background: #d4edda; padding: 2px 6px; border-radius: 3px;">Lightbox Image</span>')

    // Highlight typo:pdf tags
    html = html.replace(/&lt;typo:pdf[^\/]*\/&gt;/g,
      '<span style="background: #e3f2fd; padding: 2px 6px; border-radius: 3px;">PDF Slideshow</span>')
