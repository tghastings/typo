...
class Article < Content
  ...
  def clear_whiteboard_on_body_change
    return unless body_changed? || extended_changed?
  ...
  scope :published_at, lambda { |time_params|
    where(published: true, published_at: Article.time_delta(*time_params)).order('published_at DESC')
  }
  ...
  def set_permalink
    ...
    self.permalink = title.to_permalink if permalink.blank?
  end
  ...
  has_state(:state,
            valid_states: %i[new draft publication_pending just_published published just_withdrawn
                             withdrawn],
            initial_state: :new,
            handles: %i[withdraw post_trigger send_pings send_notifications published_at= just_published?])
  ...
  class << self
    ...
    def search_with_pagination(search_hash, paginate_hash)
      state = if search_hash[:state] && %w[no_draft drafts published withdrawn
                                           pending].include?(search_hash[:state])
                search_hash[:state]
              else
                'no_draft'
              end
      ...
      result = result.where('published_at LIKE ?', "#{search_hash[:published_at]}%") if search_hash[:published_at].present?

      result = result.where(user_id: search_hash[:user_id]) if search_hash[:user_id].present? && search_hash[:user_id].to_i.positive?

      result = result.category(search_hash[:category]) if search_hash[:category]&.to_i&.positive?
  ...
  def month_url
    format('%.2d', published_at.month)
  end

  def day_url
    format('%.2d', published_at.day)
  end
  ...
  def permalink_url_options(_nesting = false)
    ...
    format_url[0, 1] == '/' ? format_url[1..] : format_url
  end

  def permalink_url(anchor = nil, only_path = false)
    @cached_permalink_url ||= {}
    @cached_permalink_url["#{anchor}#{only_path}"] ||= blog.url_for(permalink_url_options, anchor: anchor,
                                                                                           only_path: only_path)
  end

  def param_array
    @param_array ||= [published_at.year, format('%.2d', published_at.month), format('%.2d', published_at.day),
                      permalink]
  end
  ...
  def comment_url
    blog.url_for("comments?article_id=#{id}", only_path: false)
  end

  def preview_comment_url
    blog.url_for("comments/preview?article_id=#{id}", only_path: false)
  end
  ...
  def really_send_pings(serverurl = blog.base_url, articleurl = nil)
    ...
    pingback_or_trackback_urls = html_urls
    ping_urls = weblogupdatesping_urls + pingback_or_trackback_urls
    existing_ping_urls = pings.collect(&:url)
  ...
  def self.count_by_date(year, month = nil, day = nil, _limit = nil)
    if year.blank?
  ...
  def self.find_by_permalink(params)
    date_range = time_delta(params[:year], params[:month], params[:day])
    req_params = {}
  ...
  def in_feedback_window?
    blog.sp_article_auto_close.zero? || published_at.to_i > blog.sp_article_auto_close.days.ago.to_i
  end
  ...
  def add_category(category, is_primary = false)
    categorizations.build(category: category, is_primary: is_primary)
  end
  ...
  def set_published_at
    self[:published_at] = created_at || Time.now if published && self[:published_at].nil?
  end
  ...
  def set_defaults
    set_permalink if attributes.include?('permalink') && (permalink.blank? || permalink.to_s =~ /article-draft/ || state == 'draft')
    self.allow_comments = blog.default_allow_comments if blog && allow_comments.nil?
    self.allow_pings = blog.default_allow_pings if blog && allow_pings.nil?
    true
  end
  ...
  def add_notifications
    ...
    begin
      users << user if user&.notify_watch_my_articles?
