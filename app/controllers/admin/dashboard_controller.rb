...
module Admin
  class DashboardController < Admin::BaseController
    ...
    def index
      last_venue = current_user.last_venue || Time.at(0)
      @newposts = Article.where('published = ? and published_at > ?', true, last_venue).count
      @newcomments = Feedback.where('state in (?,?) and published_at > ?', 'presumed_ham', 'ham', last_venue).count
      comments
      lastposts
      popular
      statistics
    ...
    def typo_version
      ...
      if version[0].to_i > TYPO_MAJOR.to_i
        flash.now[:error] =
          _('You are late from at least one major version of Typo. You should upgrade immediately. Download and install %s',
            "<a href='http://typosphere.org/stable.tgz'>#{_('the latest Typo version')}</a>").html_safe
      elsif version[1].to_i > TYPO_SUB.to_i
        flash.now[:warning] =
          _("There's a new version of Typo available which may contain important bug fixes. Why don't you upgrade to %s ?",
            "<a href='http://typosphere.org/stable.tgz'>#{_('the latest Typo version')}</a>").html_safe
      elsif version[2].to_i > TYPO_MINOR.to_i
        flash.now[:notice] =
          _("There's a new version of Typo available. Why don't you upgrade to %s ?",
            "<a href='http://typosphere.org/stable.tgz'>#{_('the latest Typo version')}</a>").html_safe
      end
    end
    ...
    def parse_rss(body)
      ...
      items = []
      begin
        REXML::XPath.match(xml, '//channel/link/text()').first.value
      rescue StandardError
        ''
      end
      begin
        REXML::XPath.match(xml, '//channel/title/text()').first.value
      rescue StandardError
        ''
      end
      ...
      REXML::XPath.each(xml, '//item/') do |elem|
        item = RssItem.new
        item.title = begin
          REXML::XPath.match(elem, 'title/text()').first.value
        rescue StandardError
          ''
        end
        item.link = begin
          REXML::XPath.match(elem, 'link/text()').first.value
        rescue StandardError
          ''
        end
        item.description = begin
          REXML::XPath.match(elem, 'description/text()').first.value
        rescue StandardError
          ''
        end
        item.author = begin
          REXML::XPath.match(elem, 'dc:publisher/text()').first.value
        rescue StandardError
          ''
        end
        item.date = begin
          Time.mktime(*ParseDate.parsedate(REXML::XPath.match(elem,
                                                              'dc:date/text()').first.value))
        rescue StandardError
          Time.now
        end

        item.description_link = item.description
        item.description.gsub!(%r{</?a\b.*?>}, '') # remove all <a> tags
        items << item
      end
