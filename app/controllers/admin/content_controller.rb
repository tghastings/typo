...
module Admin
  class ContentController < Admin::BaseController
    ...
    layout 'administration', except: %i[show autosave]
    ...
    def index
      ...
      @articles = Article.search_with_pagination(@search,
                                                 { page: params[:page], per_page: this_blog.admin_display_elements })
    ...
    alias resource_add category_add
    alias resource_remove category_add
    ...
    def attachment_box_add
      respond_to do |format|
        ...
        format.html do
          render partial: 'admin/content/attachment',
                 locals: { attachment_num: params[:id], hidden: false }
        end
        format.any do
          # Catch-all for XHR/API requests - return JSON
          render json: {
            id: params[:id],
            html: render_to_string(
              partial: 'admin/content/attachment',
    ...
    def do_add_or_remove_fu
      ...
      @article = Article.find(params[:id])
      send("#{attrib}=", self.class.const_get(attrib.classify).find(params["#{attrib}_id"]))
    ...
    def new_or_edit
      ...
      begin
        @article.published_at = begin
          DateTime.strptime(params[:article][:published_at],
                            '%B %e, %Y %I:%M %p GMT%z').utc
        rescue StandardError
          Time.parse(params[:article][:published_at]).utc
        end
      rescue StandardError
        nil
      end
    ...
    def set_article_title_for_autosave
      return unless @article.title.blank?
    ...
    def save_attachments
      ...
      params[:attachments].each_value do |v|
        a = attachment_save(v)
    ...
    def def_build_body
      return unless @article.body =~ /<!--more-->/
