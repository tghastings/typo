...
module Admin
  module FeedbackHelper
    ...
    def filter_link(text, _filter = '', _style = '')
      content_tag(:span, text, { class: 'label' }) unless [params[:published], params[:confirmed], params[:ham],
                                                           params[:spam], params[:presumed_ham], params[:presumed_spam]].include?('f')
    end

    def change_status(item, context = 'listing')
      status = item.state == :spam ? :ham : :spam
      link_to(_('Flag as %s', status.to_s),
              { controller: 'admin/feedback', action: 'change_state', id: item.id, context: context }, data: { turbo_method: :post })
    end
  end
end
