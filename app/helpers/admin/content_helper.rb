...
module Admin
  module ContentHelper
    def link_to_destroy_draft(record, controller_param = nil)
      return unless record.state.to_s.downcase == 'draft'

      controller_param ||= controller.controller_name
      link_to(_('Destroy this draft'),
              "/admin/#{controller_param}/destroy/#{record.id}",
              data: { confirm: _('Are you sure?') }, method: :post, class: 'btn danger')
    end
    ...
    def auto_complete_field(field_id, options = {})
      ...
      js_options[:frequency]  = options[:frequency].to_s if options[:frequency]
    ...
    def get_post_types
      ...
      return hidden_field_tag 'article[post_type]', 'read' if post_types.empty?
