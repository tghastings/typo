...
describe XmlController do
  describe 'routing' do
    ...
    it 'recognizes and generates #feed' do
      { get: '/xml/atom/feed.xml' }.should route_to(controller: 'xml', action: 'feed', type: 'feed',
                                                    format: 'atom')
    end

    it 'recognizes and generates #feed with a custom type' do
      { get: '/xml/atom/comments/feed.xml' }.should route_to(controller: 'xml', action: 'feed',
                                                             format: 'atom', type: 'comments')
    end

    it 'recognizes and generates #feed with a custom type and an id' do
      { get: '/xml/atom/comments/1/feed.xml' }.should route_to(controller: 'xml', action: 'feed',
                                                               format: 'atom', type: 'comments', id: '1')
    end
    ...
    it 'recognizes and generates #feed with sitemap type' do
      { get: '/sitemap.xml' }.should route_to(controller: 'xml', action: 'feed', type: 'sitemap',
