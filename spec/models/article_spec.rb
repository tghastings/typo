...
describe Article do
  ...
  it 'test_content_fields' do
    a = Article.new
    assert_equal %i[body extended], a.content_fields
  end

  describe '#permalink_url' do
    describe 'with hostname' do
      subject do
        Article.new(permalink: 'article-3', published_at: Time.new(2004, 6, 1)).permalink_url(nil,
                                                                                              false)
      end
    ...
    describe 'with a multibyte permalink' do
      ...
      it 'escapes the multibyte characters' do
        subject.permalink_url(nil, true).should == '/2004/06/01/%E3%83%AB%E3%83%93%E3%83%BC'
      end
    end
    ...
    describe 'with a permalink containing a space' do
      ...
      it "escapes the space as '%20', not as '+'" do
        subject.permalink_url(nil, true).should == '/2004/06/01/hello%20there'
      end
    end
    ...
    describe 'with a permalink containing a plus' do
      ...
      it 'does not escape the plus' do
        subject.permalink_url(nil, true).should == '/2004/06/01/one+two'
      end
    end
  end
  ...
  it 'test_permalink_with_title' do
    ...
    assert_equal(article,
                 Article.find_by_permalink({ year: 2004, month: 0o6, day: 0o1, title: 'article-3' }))
  ...
  describe 'with tags' do
    it 'recieves tags from the keywords property' do
      ...
      assert_equal %w[foo bar].sort, a.tags.collect(&:name).sort
    end
    ...
    it 'changes tags when changing keywords' do
      ...
      a.keywords = 'foo baz'
      a.save
      assert_equal %w[foo baz].sort, a.tags.collect(&:name).sort
    end
    ...
    it "empties tags when keywords is set to ''" do
      ...
      a.keywords = ''
      a.save
      assert_equal [], a.tags.collect(&:name).sort
    end

    it 'properly deals with dots and spaces' do
      c = Factory(:article, keywords: 'test "tag test" web2.0')
      assert_equal %w[test tag-test web2-0].sort, c.tags.collect(&:name).sort
    end
  ...
  it 'should notify' do
    ...
    Factory(:user, login: 'henri', notify_on_new_articles: true)
    Factory(:user, login: 'alice', notify_on_new_articles: true)
    ...
    assert_equal %w[alice henri], a.notify_users.collect(&:login).sort
  end
  ...
  it 'should get only ham not spam comment' do
    ...
    Factory(:spam_comment, article: article)
    article.comments.ham.should
    [ham_comment]
  ...
  describe '#published_at_like' do
    ...
    it 'should return all content for the year if only year sent' do
      Article.published_at_like(2.year.ago.strftime('%Y')).map(&:id).sort.should == [@article_two_year_ago.id,
                                                                                     @article_2_two_year_ago.id].sort
    end

    it 'should return all content for the month if year and month sent' do
      Article.published_at_like(4.month.ago.strftime('%Y-%m')).map(&:id).sort.should == [@article_four_months_ago.id,
                                                                                         @article_2_four_months_ago.id].sort
    end
