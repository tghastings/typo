...
Rails.application.config.to_prepare do
  ...
  Dir[Rails.root.join('vendor/plugins/*_sidebar/lib/*.rb')].sort.each do |sidebar_file|
...
Rails.application.config.to_prepare do
  Dir[Rails.root.join('vendor/plugins/typo_textfilter_*/lib/*.rb')].sort.each do |filter_file|
    ...
    if defined?(Typo::Textfilter) && Typo::Textfilter.const_defined?(class_name, false)
      Typo::Textfilter.send(:remove_const, class_name)
    end
    load filter_file
  end
end
