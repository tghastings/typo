  ...
  nl: {
    date: {
      formats: {
        ...
        short: '%e %b',
        long: '%e %B %Y',
        long_ordinal: ->(date) { "#{date.day}e van %B %Y" },
        only_day: '%e'
      },
      day_names: %w[Zondag Maandag Dinsdag Woensdag Donderdag Vrijdag Zaterdag],
      abbr_day_names: %w[Zo Ma Di Wo Do Vr Za],
      month_names: [nil] + %w[Januari Februari Maart April Mei Juni Juli Augustus September Oktober November
                              December],
    ...
    time: {
      formats: {
        ...
        short: '%d %b %H:%M',
        long: '%d %B %Y %H:%M',
        long_ordinal: ->(time) { "#{time.day}e van %B %Y %H:%M" },
        only_second: '%S'
      },
      datetime: {
        formats: {
      ...
      time_with_zone: {
        formats: {
          default: ->(time) { "%Y-%m-%d %H:%M:%S #{time.formatted_offset(false, 'UTC')}" }
        }
      },
      am: "'s ochtends",
      pm: "'s middags"
    ...
    datetime: {
      distance_in_words: {
        ...
        less_than_x_seconds: { zero: 'minder dan een seconde', one: 'minder dan 1 seconde',
                               other: 'minder dan {{count}} secondes' },
        x_seconds: { one: '1 seconde', other: '{{count}} secondes' },
        less_than_x_minutes: { zero: 'minder dan een minuut', one: 'minder dan 1 minuut',
