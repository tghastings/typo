...
Localization.define('de_DE') do |l|
  ...
  l.store 'Currently this article has the following resources',
          'Aktuell sind folgende Ressourcen dem Artikel zugeordnet'
  ...
  l.store 'Typo can notify you when new articles or comments are posted',
          'Typo kann Sie benachrichtigen, wenn neue Artikel oder Kommentare angelegt werden'
  l.store 'Source Email', 'Email Adresse'
  l.store 'Email address used by Typo to send notifications',
          'Email Adresse, die Typo beim Versenden von Benachrichtigungen verwenden soll'
  l.store 'Enabling spam protection will make typo compare the IP address of posters as well as the contents of their posts against local and remote blacklists. Good defense against spam bots',
          'Bei Aktivierung des Spamschutzes wird Typo sowohl die IP Adresse des Autors als auch den Inhalt seiner Veröffentlichung gegen lokale und entfernte Blacklisten vergleichen. Gute Abwehr von Spambots'
  ...
  l.store 'Typo can (optionally) use the %s spam-filtering service.  You need to register with Akismet and receive an API key before you can use their service.  If you have an Akismet key, enter it here',
          'Typo kann (optional) den %s Spam-Filterdienst verwenden. Sie müssen sich dort registriert und einen API Key erhalten haben, bevor Sie diesen Dienst nutzen können. Wenn Sie einen solchen Key haben, geben Sie ihn hier ein'
  ...
  l.store "This setting allows you to disable trackbacks for every article in your blog.  It won't remove existing trackbacks, but it will prevent any further attempt to add a trackback anywhere on your blog.",
          'Diese Option erlaubt es Ihnen, Trackbacks für alle Artikel im gesamten Blog zu deaktivieren. Dadurch werden zwar keine bereits existierenden Trackbacks entfernt, aber alle zukünftig irgendwo in Ihrem Blog eintreffenden Trackbacks werden abgewiesen.'
  l.store 'Disable comments after', 'Kommentare abschalten nach'
  l.store 'days', 'Tagen'
  l.store 'Set to 0 to never disable comments',
          'Wert 0 bewirkt, dass die Möglichkeit für Kommentare immer bestehen bleibt'
  ...
  l.store 'Typo will automatically reject comments and trackbacks which contain over a certain amount of links in them',
          'Typo kann automatisch Kommentare und Trackbacks abweisen, die mehr als eine bestimmte Anzahl von Links enthalten'
  ...
  l.store 'Drag some plugins here to fill your sidebar',
          'Ziehen Sie Plugins hierher, um sie in die Seitenleiste aufzunehmen'
  ...
  l.store "Drag and drop to change the sidebar items displayed on this blog.  To remove items from the sidebar just click remove  Changes are saved immediately, but not activated until you click the 'Publish' button",
          "Verwenden Sie Drag&Drop, um die Einträge der Seitenleiste ihres Blogs zu ändern. Um Einträge zu löschen, klicken Sie auf 'remove'. Änderungen sind hier sofort sichtbar, werden aber erst permanent aktiviert, wenn Sie 'Änderungen veröffentlichen' klicken."
  ...
  l.store 'Send notification messages when new articles are posted',
          'Benachrichtigung schicken, wenn neue Artikel veröffentlicht werden'
  l.store 'Send notification messages when comments are posted',
          'Benachrichtigung schicken, wenn neue Kommentare eintreffen'
  ...
  l.store "This comment has been flagged for moderator approval.  It won't appear on this blog until the author approves it",
          'Dieser Kommentar wurde für die Moderatorfreigabe markiert. Er wird erst nach Freigabe durch den Moderator in diesem Blog erscheinen'
  ...
  l.store 'Currently this article is listed in following categories',
          'Aktuell ist dieser Artikel den folgenden Kategorien zugeordnet'
  ...
  l.store 'Jabber account to use when sending Jabber notifications',
          'Jabber Account für das Senden von Jabber Benachrichtigungen'
  ...
  l.store 'The below settings act as defaults when you choose to publish an enclosure with iTunes metadata',
          'Folgende Einstellungen wirken als Voreinstellungen, wenn Sie einen Anhang mit iTunes Metadaten veröffentlichen'
  ...
  l.store 'You can enable site wide feedback moderation. If you do so, no comment or trackback will appear on your blog unless you validate it',
          'Sie können die Moderation von Kommentaren auf der gesamten Website aktivieren. Dann erscheinen keine Kommentare oder Trackbacks in Ihrem Blog, die sie nicht überprüft haben'
  l.store 'You can optionally disable non-Ajax comments. Typo will always use Ajax for comment submission if Javascript is enabled, so non-Ajax comments are either from spammers or users without Javascript.',
          'Sie können optional non-Ajax Kommentare verbieten. Typo verwendet immer Ajax für die Übertragung von Kommentaren, sofern Javascript eingeschaltet ist. non-Ajax Kommentare stamme somit entweder von Spammern oder von Anwendern ohne aktiviertes Javascript.'
  l.store 'by', 'bei'
