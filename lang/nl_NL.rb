...
Localization.define('nl_NL') do |l|
  ...
  l.store 'An email has been successfully sent to your address with your new password',
          'Er is je met succes een e-mail gestuurd met uw nieuwe wachtwoord'
  ...
  l.store 'Please review and save the settings before continuing',
          'Controleer de instellingen en sla ze op voor je verder gaat'
  ...
  l.store 'It seems something went wrong. Maybe some of your sidebars are actually missing and you should either reinstall them or remove them manually',
          'Het lijkt er op dat er iets fout ging. Misschien ontbreken sommige zijbalken en moet je ze opnieuw installeren of handmatig verwijderen'
  ...
  l.store "There are no %s yet. Why don't you start and create one?",
          'Er zijn nog geen %s. Waarom begin je er niet een te maken?'
  l.store 'or', 'of'
  l.store 'Save', 'Bewaar'
  ...
  l.store 'it has been illegally reproduced and without proper authorization',
          'is het illegaal en zonder de juiste toestemming gekopieerd'

  # app/models/blog.rb
  l.store 'You need a permalink format with an identifier : %%month%%, %%year%%, %%day%%, %%title%%',
          'Je hebt een permalink formaat nodig met een identifier: %%month%%, %%year%%, %%day%%, %%title%%'
  l.store "Can't end in .rss or .atom. These are reserved to be used for feed URLs",
          'Mag niet eindigen met .rss of .atom. Die zijn gereserveerd voor feed URLs'
  ...
  l.store 'Excerpts are posts summaries that are shown on your blog homepage only but won’t appear on the post itself',
          'Uittreksels zijn een samenvatting van een post die alleen op de homepage staat, maar niet in de post zelf staan'
  ...
  l.store 'This place gives you a quick overview of what happens on your Typo blog and what you can do. Maybe will you want to %s, %s or %s.',
          'Deze pagina geeft je een snel overzicht van wat er gebeurt op je Type blog en wat je doen kunt. Misschien wil je %s, %s of %s'
  ...
  l.store "No posts yet, why don't you start and write one",
          'Nog geen berichten, waarom begin je er niet een te schrijven'
  ...
  l.store '%d articles and %d comments were posted since your last connexion',
          '%d artikels en %d commentaren zijn geplaatst sinds je laatste bezoek'
  ...
  l.store 'This comment by <strong>%s</strong> was flagged as spam, %s?',
          'Dit commentaar van <strong>%s</strong> was gemarkeerd als spam, %s?'
  ...
  l.store 'You can enable site wide feeback moderation. If you do so, no comment or trackback will appear on your blog unless you validate it',
          'Je kunt voor de hele site feedback controle aanzetten. Als je dat doet zal geen enkel commentaar of trackback op je blog verschijnen totdat je het goedkeurt'
  ...
  l.store 'Typo can notify you when new articles or comments are posted',
          'Typo can je notificeren wanneer nieuwe artikelen of commentaren gepost worden'
  l.store 'Source Email', 'Bron e-mail'
  l.store 'Email address used by Typo to send notifications',
          'E-mail adres dat Typo gebruikt om notificaties te versturen'
  l.store 'Enabling spam protection will make typo compare the IP address of posters as well as the contents of their posts against local and remote blacklists. Good defense against spam bots',
          'Het aanzetten van spambescherming zorgt dat typo het IP-adres en de inhoud van reacties vergelijkt met lokale en centrale zwarte lijsten. Een goede bescherming tegen spam robots'
  ...
  l.store 'Typo can (optionally) use the %s spam-filtering service.  You need to register with Akismet and receive an API key before you can use their service.  If you have an Akismet key, enter it here',
          'Typo kan (optioneel) de %s spam-filter dienst gebruiken. Je moet een registratie bij Akismet hebben en een API sleutel voordat je deze dienst kunt gebruiken. Als je een Akismet sleutel hebt, vul die dan hier in'
  ...
  l.store "This setting allows you to disable trackbacks for every article in your blog.  It won't remove existing trackbacks, but it will prevent any further attempt to add a trackback anywhere on your blog.",
          'Deze instelling zorgt er voor dat bij geen enkel artikel in je blog trackbacks gebruikt kunnen worden. Het zal bestaande trackbacks niet verwijderen, maar het zal voorkomen dat nieuwe trackbacks worden toegevoegd.'
  ...
  l.store 'Typo will automatically reject comments and trackbacks which contain over a certain amount of links in them',
          'Typo zal automatisch commentaren en trackbacks verwijderen die meer dan een bepaald aantal links bevatten'
  ...
  l.store "You can allow users to register to your blog. By default, they will register as contributors, an unpriviledged account level which grant them no rights but own a profile on the site. If you don't want users to register, you can thus add them by yourself in the users part of this admin.",
          'Je kunt gebruikers toestaan zich te registreren bij je blog. Standaard zullen ze contributors worden, een soort account zonder rechten behalve het hebben van een profiel op de site. Als je niet wilt dat gebruikers zich registreren kun je ze altijd zelf toevoegen in het gebruikers deel.'
  ...
  l.store 'You can use your Google Feedburner account instead of Typo feed URL. To enable this, fill this form with your Feedburner ID.',
          'Je kunt je Google Feedburner account gebruiken in plaats van de Typo feed URL. Vul daartoe in dit formulier je Feedburner ID in.'
  ...
  l.store 'Unchecking this box will add <code>noindex, follow</code> meta tags in every category page, removing them from search engines and preventing duplicate content issues',
          'Het uitzetten van dit vinkje zal de meta-tags <code>noindex, follow</code> toevoegen aan elke categorie pagina, zodat die niet door zoekmachines ge&iuml;ndexeerd wordt; dit voorkomt dubbele hits'
  l.store 'Index tags', ''
  l.store 'Unchecking this box will add <code>noindex, follow</code> meta tags in every tags page, removing them from search engines and preventing duplicate content issues',
          'Het uitzetten van dit vinkje zal de meta-tags <code>noindex, follow</code> toevoegen aan elke tag pagina, zodat die niet door zoekmachines ge&iuml;ndexeerd wordt; dit voorkomt dubbele hits'
  l.store 'Robots.txt', 'Robots.txt'
  l.store "You robots.txt file is not writeable. Typo won't be able to write it",
          'Je robots.txt bestand is niet schrijfbaar. Typo kan het niet wijzigen'
  ...
  l.store "When publishing articles, Typo can send trackbacks to websites that you link to. This should be disabled for private blogs as it will leak non-public information to sites that you're discussing. For public blogs, there's no real point in disabling this.",
          "Wanneer je een artikel publiceert kan Typo een trackback sturen naar de sites waaraan je linkt. Dit zou voor priv&eacute;-blogs uit moeten staan, omdat het niet-publieke informatie 'lekt' naar sites die je bediscussieert. Voor publieke blogs is er geen goede reden om het uit te zetten."
  ...
  l.store "Drag and drop to change the sidebar items displayed on this blog.  To remove items from the sidebar just click remove  Changes are saved immediately, but not activated until you click the 'Publish' button",
          "Sleep dingen heen en weer om de zijbalk van dit blog aan te passen. Om dingen te verwijderen klik je op verwijderen  Wijzigen worden automatisch opgeslagen, maar niet geactiveerd totdat je de 'Publiceer' knop klikt"
  ...
  l.store 'You can download third party themes from officially supported %s ',
          "Je kunt thema's van derden downloaden van officieel ondersteunde %s "
  ...
  l.store 'To install a theme you  just need to upload the theme folder into your themes directory. Once a theme is uploaded, you should see it on this page.',
          'Om een thema te installeren moet je het uploaden in de themes map. Zodra het is geupload zou je het op deze pagina moeten zien.'
  ...
  l.store 'Send notification messages when new articles are posted',
          'Zend notificaties wanneer nieuwe artikelen worden gepubliceerd'
  l.store 'Send notification messages when comments are posted',
          'Zend notificaties wanneer commentaren geplaatst worden'
  ...
  l.store "This comment has been flagged for moderator approval.  It won't appear on this blog until the author approves it",
          'Dit commentaar is gemarkeerd voor goedkeuring. Het zal niet getoond worden totdat de auteur het goedkeurt.'
  ...
  l.store 'Oops, something wrong happened, the comment could not be saved',
          'Er is iets mis gegaan en je reactie is niet bewaard'
  ...
  l.store 'Use the following link to trackback from your own site',
          'Gebruik de volgende link voor een trackback van jouw site'
  ...
  l.store 'Welcome to your %s blog setup. Just fill in your blog title and your email, and Typo will take care of everything else',
          'Welkom bij je %s blog setup. Vul een titel voor je blog in, en een e-mailadres, en Typo zorgt voor de rest'
  ...
  l.store "Don't lose the mail sent at %s or you won't be able to login anymore",
          'Verlies de e-mail verzonden naar %s, want anders kun je niet meer inloggen'
