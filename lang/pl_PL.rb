...
Localization.define('pl_PL') do |l|
  ...
  l.store '%d articles and %d comments were posted since your last connexion',
          '%d artykułów i %d komentarzy zostało opublikowanych od twojego ostaniego połączenia'
  ...
  l.store 'You can enable site wide feeback moderation. If you do so, no comment or trackback will appear on your blog unless you validate it',
          'Możesz włączyć globalną moderację komentarzy. W takim przypadku żaden komentarz nie ukaże się na blogu aż do momentu jego akceptacji.'
  ...
  l.store 'Typo can notify you when new articles or comments are posted',
          'Typo może wysyłać powiadomienia o nowych artykułach bądź komentarzach'
  ...
  l.store 'Enabling spam protection will make typo compare the IP address of posters as well as the contents of their posts against local and remote blacklists. Good defense against spam bots',
          'Włączenie ochrony przed spamem sprawi, iż Typo będzie porównywać adresy IP nadawców oraz treść ich postów z lokalnymi i zdalnymi czarnymi listami. To dobra obrona przed spam botami.'
  ...
  l.store 'Typo can (optionally) use the %s spam-filtering service.  You need to register with Akismet and receive an API key before you can use their service.  If you have an Akismet key, enter it here',
          'Typo może (opcjonalnie) stosować usługę %s do filtrowania spamu. Musisz zarejestrować się w serwisie Akismet by otrzymać klucz API nim będzie można używać tej usługi. Jeśli posiadasz klucz API Akismet, wprowadź go tutaj'
  ...
  l.store "This setting allows you to disable trackbacks for every article in your blog.  It won't remove existing trackbacks, but it will prevent any further attempt to add a trackback anywhere on your blog.",
          'Ta opcja pozwala na wyłączenie trackbacków we wszystkich artykułach. Nie usunie jednak istniejących trackbacków, a jedynie zabroni dodawania nowych.'
  ...
  l.store 'Typo will automatically reject comments and trackbacks which contain over a certain amount of links in them',
          'Typo automatycznie odrzuca komentarze i trackbacki, które zawierają większą, niż podana, liczbę łączy'
  ...
  l.store "When publishing articles, Typo can send trackbacks to websites that you link to. This should be disabled for private blogs as it will leak non-public information to sites that you're discussing. For public blogs, there's no real point in disabling this.",
          'Publikując artykuły, Typo może powiadomić strony, do których podasz łącza. Opcja ta powinna być wyłączona w przypadku blogów osobistych, gdyż może doprowadzić do wycieku prywatnych informacji. W przypadku blogów publicznych wyłączenie tej opcji nie ma większego sensu.'
  ...
  l.store "Drag and drop to change the sidebar items displayed on this blog.  To remove items from the sidebar just click remove  Changes are saved immediately, but not activated until you click the 'Publish' button",
          "Przeciągnij i upuść pozycje, które mają być wyświetlone na blogu. By usunąć pozycję naciśnij *Usuń*. Zmiany są zapisywane od razu, lecz nie są aktywne do momentu kliknięcia 'Publikuj zmiany'"
  ...
  l.store 'Currently this article is listed in following categories',
          'Artykuł jest opublikowany w następujących kategoriach'
  ...
  l.store 'The below settings act as defaults when you choose to publish an enclosure with iTunes metadata',
          'Poniższe ustawienia podawane są jako domyślne jeśli publikacja będzie zawierać metadane iTunes'
  ...
  l.store 'This option let you choose between the simple admin interface or the complete one, displaying much more options and therefore more complicated to use. For advanced users only!',
          'Ta opcja pozwala wybrać prosty lub pełny panel administracyjny. Pełny panel administracyjny zawiera więcej opcji przez co jest bardziej skomplikowany. Tylko dla zaawansowanych użytkowników!'
  ...
  l.store 'You can optionally disable non-Ajax comments. Typo will always use Ajax for comment submission if Javascript is enabled, so non-Ajax comments are either from spammers or users without Javascript.',
          'Można opcjonalnie wyłączyć nie-Ajaxowe komentarze. Typo zawsze używa technologii Ajax do przesyłania komentarzy - o ile Javascript jest włączony. Przeważnie komentarze nie-Ajaxowe pochodzą od spamerów lub użytkowników bez Javascript.'
