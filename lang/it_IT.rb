...
Localization.define('it_IT') do |l|
  ...
  l.store 'Error occurred while updating Content Type.',
          "Si e' verificato un errore mentre aggiornavo il tipo di contenuto."
  ...
  l.store 'You can enable site wide feeback moderation. If you do so, no comment or trackback will appear on your blog unless you validate it',
          "Puoi abilitare in modo globale la moderazione dei feedback. Se fai cio', nessun commento o trackback apparira' sul tuo blog se tu non lo autorizzi."
  ...
  l.store 'Typo can notify you when new articles or comments are posted',
          "Il blog puo' notificarti l'inserimento di un nuovo articolo e/o commento"
  ...
  l.store 'Enabling spam protection will make typo compare the IP address of posters as well as the contents of their posts against local and remote blacklists. Good defense against spam bots',
          "Abilitando la protezione contro lo spam fa si che il blog compari gli IP di chi invia i commenti e anche il loro contenuto con una blacklist remota. E' una buona difesa contro gli spam robot"
  ...
  l.store 'Typo will automatically reject comments and trackbacks which contain over a certain amount of links in them',
          "Il blog cancellera' automaticamente commenti e trackbacks che contengono un certo numero di link"
  ...
  l.store "When publishing articles, Typo can send trackbacks to websites that you link to. This should be disabled for private blogs as it will leak non-public information to sites that you're discussing. For public blogs, there's no real point in disabling this.",
          "Quando pubblichi gli articoli, e' possibile inviare trackback ai siti di cui fornisci l'url. E' possibile disabilitare questa funzione nel caso di blog privato di cui non si vogliono condividere le informazioni. Per blog pubblici non esiste una necessita' reale di disabilitare questa funzione."
  ...
  l.store "Drag and drop to change the sidebar items displayed on this blog.  To remove items from the sidebar just click remove  Changes are saved immediately, but not activated until you click the 'Publish' button",
          "Trascina e rilascia per cambiare gli elementi visualizzati nella sidebar del tuo blog. Per rimuorverli clicca su annulla Cambiamenti perche' comunque non sono salvati automaticamente ma devi cliccare il bottone 'Pubblica'"
  ...
  l.store 'Send notification messages when new articles are posted',
          'Invia notifiche quando viene pubblicato un nuovo articolo'
  l.store 'Send notification messages when comments are posted',
          'Invia notifiche quando viene inserito un nuovo commento'
  ...
  l.store 'Use the following link to trackback from your own site',
          'Usa il link seguente per fare un trackback dal tuo sito'
  ...
  l.store 'Currently this article is listed in following categories',
          "Questo articolo e' presente nelle seguenti categorie"
  ...
  l.store 'The below settings act as defaults when you choose to publish an enclosure with iTunes metadata',
          'Le impostazioni di seguito agiscono come default quando si sceglie di pubblicare delle risorse con metadati iTunes'
  l.store 'Themes', 'Temi'
  l.store 'Things you can do', 'Cose che puoi fare'
  l.store "This comment has been flagged for moderator approval. It won't appear on this blog until the author approves it",
          "Questo commento e' stato segnalato per essere moderato. Non apparira' finche' gli amministratori non l'approvano"
  l.store 'This option let you choose between the simple admin interface or the complete one, displaying much more options and therefore more complicated to use. For advanced users only!',
          'Queste opzioni ti permettono di scegliere tra una amministrazione semplice o quella completa, mostrando maggiori opzioni anche complicate da usare. Solo per utenti esperti!!'
  l.store "This setting allows you to disable trackbacks for every article in your blog. It won't remove existing trackbacks, but it will prevent any further attempt to add a trackback anywhere on your blog.",
          "Questo settaggio ti permette di disabilitare i trackbacks sugli articoli. Non rimuovera' i trackback esistenti, ma ti proteggera' in futuro."
  l.store 'Toggle Extended Content', 'Mostra contenuto esteso'
  l.store 'Type', 'Tipo'
  l.store 'Typo admin', 'amministrazione typo'
  l.store 'Typo can (optionally) use the %s spam-filtering service. You need to register with Akismet and receive an API key before you can use their service. If you have an Akismet key, enter it here',
          "Il blog puo' opzionalmente utilizzare %s come filtro spam. Registrati ad Akismet e reiceverai una chiave API prima di poter utilizzare il loro servizio. Se hai una chiave Akismet, inseriscila qui"
  ...
  l.store 'You can optionally disable non-Ajax comments. Typo will always use Ajax for comment submission if Javascript is enabled, so non-Ajax comments are either from spammers or users without Javascript.',
          "Puoi disabilitare i commenti non Ajax. Il blog usera' Ajax per l'invio dei commenti se i Javascript sono abilitati, in questo modo i commenti non Ajax saranno bloccati dagli spammer e dagli utenti senza javascript."
