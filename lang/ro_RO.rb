...
Localization.define('ro_RO') do |l|
  ...
  l.store 'Typo can notify you when new articles or comments are posted',
          'Typo te poate alerta la publicarea de noi articole sau comentarii'
  l.store 'Source Email', 'Expeditor email'
  l.store 'Email address used by Typo to send notifications',
          'Adresa email folosită de Typo pentru a trimite alerte prin email'
  l.store 'Enabling spam protection will make typo compare the IP address of posters as well as the contents of their posts against local and remote blacklists. Good defense against spam bots',
          'La activarea protecției anti-spam, Typo va compara adresa IP a utilizatorului care publică un articol, cît și conținutul articolului publicat, cu niște liste negre locale sau din altă parte. Reprezintă o bună apărare împotriva unui spam bot '
  ...
  l.store 'Typo can (optionally) use the %s spam-filtering service.  You need to register with Akismet and receive an API key before you can use their service.  If you have an Akismet key, enter it here',
          'Opțional, Typo poate folosi serviciul %s de filtrare spam. Trebuie să te înregistrezi la Akismet și să primești o cheie API pentru a putea folosi acest serviciu. Dacă ai deja o cheie Akismet, introdu-o aici'
  ...
  l.store 'Typo will automatically reject comments and trackbacks which contain over a certain amount of links in them',
          'Typo va respinge automat comentarii și retrolegături care conțin mai mult decît un anumit număr de legături'
  ...
  l.store "When publishing articles, Typo can send trackbacks to websites that you link to. This should be disabled for private blogs as it will leak non-public information to sites that you're discussing. For public blogs, there's no real point in disabling this.",
          'Cînd publici un articol, Typo poate trimite automat retrolegături site-urilor pe care le specifici. În cazul unui blog personal, această opțiune ar trebui dezactivată, ca să nu deconspire informații private. Dacă blogul e public, opțiunea aceasta poate rămîne activă.'
  ...
  l.store "Drag and drop to change the sidebar items displayed on this blog.  To remove items from the sidebar just click remove  Changes are saved immediately, but not activated until you click the 'Publish' button",
          "Trage și lasă obiecte pe bara laterală pentru a o modifica în blog. Pentru a șterge obiecte din bara laterală, apasă pe 'șterge'. Modificările se salvează imediat, dar nu vor fi activate pînă cînd apeși butonul 'publică'"
  ...
  l.store "This comment has been flagged for moderator approval.  It won't appear on this blog until the author approves it",
          'Acest comentariu a fost marcat pentru moderare. El nu va apărea în blog înainte de a fi aprobat.'
  ...
  l.store 'Use the following link to trackback from your own site',
          'Folosește link-ul următor pentru a realiza o retrolegătură de la site-ul tău'
  ...
  l.store 'Jabber account to use when sending Jabber notifications',
          'Contul de Jabber folosit pentru a trimite alerte prin Jabber'
  ...
  l.store 'The below settings act as defaults when you choose to publish an enclosure with iTunes metadata',
          'Setările de mai jos vor reprezenta valorile implicite atunci cînd vei publica un enclosure cu metainformații pentru iTunes'
  ...
  l.store 'You can optionally disable non-Ajax comments. Typo will always use Ajax for comment submission if Javascript is enabled, so non-Ajax comments are either from spammers or users without Javascript.',
          'Opțional, puteți dezactiva comentariile non-AJAX. Typo va folosi întotdeauna AJAX pentru trimiterea comentariilor dacă funcționează Javascript la utilizator, astfel că orice comentariu care nu vine prin AJAX trebuie să vină fie de la spammeri, fie de la vizitatori fără Javascript.'
  l.store 'by', 'de'
  l.store 'on', 'la'
