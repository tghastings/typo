...
Localization.define('he_IL') do |l|
  ...
  l.store 'You can enable site wide feeback moderation. If you do so, no comment or trackback will appear on your blog unless you validate it',
          'תוכל לאפשר ביקורת על המשוב בכל האתר. אם תבצע זאת, לא תוצג שום תגובה או עוקב-חזרה בבלוג שלך אלא אם תאשר אותה.'
  ...
  l.store 'Typo can notify you when new articles or comments are posted',
          'Typo יכולה לעדכן אותך בעת שמאמרים חדשים או תגובות נשלחות.'
  ...
  l.store 'Enabling spam protection will make typo compare the IP address of posters as well as the contents of their posts against local and remote blacklists. Good defense against spam bots',
          'אפשור הגנת ספאם תגרום ל-Typo להשוות כתובות ה-IP של השולחים ואת תוכן השליחה כנגד רשימות שחורות מקומיות או מרוחקות - הגנה טובה כנגד רובוטי ספאם.'
  ...
  l.store 'Typo can (optionally) use the %s spam-filtering service.  You need to register with Akismet and receive an API key before you can use their service.  If you have an Akismet key, enter it here',
          'תוכל להשתמש בשירות מסנן ספאם של %s. עליך להירשם Akisemt מפתח API לפני שתוכל להשתמש בשירות. אם כבר יש לך מפתח, רשום אותו כאן.'
  ...
  l.store "This setting allows you to disable trackbacks for every article in your blog.  It won't remove existing trackbacks, but it will prevent any further attempt to add a trackback anywhere on your blog.",
          'הגדרה זו מאפשרת לך לבטל עוקבים-חזרה לכל כתבה בבלוג שלך. היא לא תסיר עוקבים-חזרה קיימים, אבל תמנע נסיונות להוספת עוקב-חזרה בכל הכתבות הבלוג שלך.'
  ...
  l.store 'Typo will automatically reject comments and trackbacks which contain over a certain amount of links in them',
          'Typo תדחה אוטומטית תגובות ועוקבים-חזרה המכילים את כמות הקישורים מסויימת'
  ...
  l.store "You can allow users to register to your blog. By default, they will register as contributors, an unpriviledged account level which grant them no rights but own a profile on the site. If you don't want users to register, you can thus add them by yourself in the users part of this admin.",
          'תוכל לאפשר למשתמשים להירשם לבלוג שלך. כברירת מחדל, הם יירשמו כתורמים - חשבון שאינו נותן הרשאות מיוחדות אלא רק את הזכות להחזיק  חשבון באתר. אם אינך רוצה שהמשתמשים יוכלו להירשם בעצמם, תוכל להוסיף אותם בעצמך כחללק מדף זה'
  ...
  l.store "When publishing articles, Typo can send trackbacks to websites that you link to. This should be disabled for private blogs as it will leak non-public information to sites that you're discussing. For public blogs, there's no real point in disabling this.",
          'בעת פרסום במאמר, Typo יכולה לשלוח עוקבים-חזרה לאתרים אליהם אתה מקשר. מומלץ לבטל זאת לבלוגים פרטיים משום שמידע פרטי על האתרים אליהם אתה מקשר עלול לדלוף. לבלוגים ציבוריים, אין טעם לבטל זאת.'
  ...
  l.store "Drag and drop to change the sidebar items displayed on this blog.  To remove items from the sidebar just click remove  Changes are saved immediately, but not activated until you click the 'Publish' button",
          "גרור והשלך כדי לשנות את פריטי תיבת הצד המוצגת בבלוג שלך. כדי להסיר פריטים מתיבת הצד, לחץ על הסר. השינויים נשמרים מיידית אבל לא מופעלים עד שתלחץ על הכפתור 'פרסם'."
  ...
  l.store 'To install a theme you  just need to upload the theme folder into your themes directory. Once a theme is uploaded, you should see it on this page.',
          'כדי להתקין עיצוב עליך תצטרך להעלות אותא אל ספריית העיצובים שלך (Themes). ברגע שהעיצוב הועלה תוכל לראות בדף זה.'
  ...
  l.store "This comment has been flagged for moderator approval.  It won't appear on this blog until the author approves it",
          'תגובה זו הועברה לביקורת. היא לא תופיע בבלוג עד אשר הכותב יאשר אותה'
  ...
  l.store 'By default, Typo generates static HTML pages for your posts. However, if you plan to publish posts in the futur, you may want to use semi dynamic caching',
          'כברירת מחדל, Typo מייצרת  דפי HTML סטטיים עבור הכתבות שלך. למרות זאת, אם אתה מעוניין לפרסם כתבות בעתיד, ייתכן כי תרצי מטמון חצי דינמי'
  ...
  l.store 'The below settings act as defaults when you choose to publish an enclosure with iTunes metadata',
          'ההגדרות למטה משמשות כברירת מחדל בעת שאתה בוחר לפרסם מעטפת עם מידע-על של iTunes'
  l.store 'Themes', 'עיצובים'
  l.store "There is no %s yet. Why don't you start and create one?", 'אין עדיין %s. למה לא תיצור אחד חדש?'
  l.store 'This place gives you a quick overview of what happens on your Typo blog and what you can do. Maybe will you want to %s , %s or %s.',
          'כאן תמצא סקירה קצרה על מה שקורה בבלוג שלך ומה תוכל לעשות. אולי תרצה %s, %s , %s'
  ...
  l.store 'You can optionally disable non-Ajax comments. Typo will always use Ajax for comment submission if Javascript is enabled, so non-Ajax comments are either from spammers or users without Javascript.',
          'תוכל לבטל תגובות ללא Ajax‎ . המע‎‎רכת תמיד משתמשת ב-Ajax עבור תגובות אם Javascript מאופשר, ולכן תגובות ללא Ajax הם תמיד מספאמרים או ממשתמשים ללא Javascript'
