...
module ActionWebService # :nodoc:
  module TestInvoke # :nodoc:
    module InstanceMethods # :nodoc:
      ...
      def invoke_direct(method_name, *)
        invoke_detailed('api', 'api', method_name, *)
      end

      alias invoke invoke_direct

      # invoke the specified API method on the specified service
      def invoke_delegated(service_name, method_name, *)
        invoke_detailed(service_name.to_s, service_name, method_name, *)
      end

      # invoke the specified layered API method on the correct service
      def invoke_layered(service_name, method_name, *)
        invoke_detailed('api', service_name, method_name, *)
      end

      def invoke_detailed(action, service_name, method_name, *)
        prepare_request(action, service_name, method_name, *)
      ...
      def prepare_request(action, service_name, api_method_name, *)
        @request.recycle!
        @request.request_parameters['action'] = action
        @request.env['REQUEST_METHOD'] = 'POST'
        @request.env['HTTP_CONTENT_TYPE'] = 'text/xml'
        @request.env['RAW_POST_DATA'] = encode_rpc_call(service_name, api_method_name, *)
        case protocol
      ...
      def encode_rpc_call(service_name, api_method_name, *args)
        ...
        if method&.expects && args.length != method.expects.length
          raise ArgumentError,
                "wrong number of arguments for rpc call (#{args.length} for #{method.expects.length})"
        end
      ...
      def decode_rpc_response
        _, return_value = protocol.decode_response(@response.body)
      ...
      def public_method_name(service_name, api_method_name)
        ...
        if @controller.web_service_dispatching_mode == :layered && protocol.is_a?(ActionWebService::Protocol::XmlRpc::XmlRpcProtocol)
          format('%s.%s', service_name.to_s, public_name)
        else
          public_name
        end
      end
