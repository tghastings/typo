  ...
  module Dispatcher # :nodoc:
    module ActionControllerX # :nodoc:
      def self.included(base) # :nodoc:
        ...
        base.add_web_service_api_callback do |klass, _api|
          klass.class_eval 'def api; dispatch_web_service_request; end', __FILE__, __LINE__ if klass.web_service_dispatching_mode == :direct
        end
        base.add_web_service_definition_callback do |klass, name, _info|
          if klass.web_service_dispatching_mode == :delegated
            klass.class_eval "def #{name}; dispatch_web_service_request; end", __FILE__, __LINE__
          elsif klass.web_service_dispatching_mode == :layered
            klass.class_eval 'def api; dispatch_web_service_request; end', __FILE__, __LINE__
          end
        end
      ...
      module InstanceMethods # :nodoc:
        ...
        def send_web_service_error_response(ws_request, exception)
          if ws_request
            exception = DispatcherError.new('Internal server error (exception raised)') unless self.class.web_service_exception_reporting
            ...
            ws_response = ws_request.protocol.encode_response("#{public_method_name}Response", exception,
                                                              return_type, ws_request.protocol_options)
        ...
        def log_request(ws_request, body)
          ...
          params = if api_method&.expects
                     api_method.expects.zip(params).map { |type, param| "#{type.name}=>#{param.inspect}" }
                   else
                     params.map(&:inspect)
                   end
