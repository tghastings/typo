...
module BareMigration
  ...
  module ClassMethods
    ...
    def find_and_update(find_type = :all, *rest, &update_block)
      transaction do
        records = find_type == :all ? all : where(rest.first || {})
...
module ActiveRecord
  class Migration
    ...
    def self.modify_schema(method, *args)
      case method.to_s
      when 'create_table'
        block = args.last.is_a?(Proc) ? args.pop : proc { |_t| }
    ...
    def self.modify_tables_and_update(*colspecs, &block)
      colspecs = [colspecs] unless colspecs.first.is_a?(Array)
      begin
        updated_classes = []
        ...
        colspecs.reverse.each do |(method, table, column, *rest)|
          modify_schema(opposite_of(method), table, column, *rest)
