...
class String
  ...
  def to_title(item, settings, params)
    ...
    if s =~ /(%excerpt%)/ && item.respond_to?(:body)
      s = s.gsub('%excerpt%',
                 (item.body || '').strip_html.slice(0, 160))
    end
    ...
    s = s.gsub('%categories%', article.categories.map(&:name).join(', ')) if s =~ /(%categories%)/ && item.respond_to?(:categories)

    s = s.gsub('%tags%', article.tags.map(&:display_name).join(', ')) if s =~ /(%tags%)/ && item.respond_to?(:tags)
    ...
    s.gsub('%currentyear%', Time.now.year.to_s)
  end
